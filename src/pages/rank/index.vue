    ...
    <div class="hs-width-00100 hs-height-60 hs-row-center-center hs-position-relative">
      <ul class="hs-ini hs-row- hs-child-line-height-30 hs-child-background-color-white hs-child-color-warning hs-child-padding-left-right-15 hs-child-margin-left-5 hs-child-margin-right-5 hs-child-border-radius-10 hs-child-cursor-pointer">
        <li v-for="(rank_title_val,index) in rank_title_msg"
            ...
            :style="{'background-color':cur_rank_title === rank_title_val.title_name?'rgb(230,162, 60)':'','color':cur_rank_title === rank_title_val.title_name?'#fff':''}"
        >
            {{rank_title_val.title_name}}
        </li>
      </ul>
      <div class="hs-color-white-05 hs-font-size-09 hs-position-absolute hs-right-20" v-show="update_time!==''">最近更新: {{ update_time }} <span class="hs-color-warning hs-cursor-pointer" @click="on_update_cache">更新</span></div>
    </div>
    <!--歌曲列表-->
...
if(wyy_music_cache_log){ // 如果存在本地缓存，根据缓存记录,获取上次选中的标题信息，从sqlite数据库取数据
  ...
  new Http({  // 发送http请求
    ...
    // 插入数据到数据库
    await musicStore.db.delete(`DELETE FROM ${rank_title_msg[0].table_name}`) // 新增前先清空表数据
  ...
  const rank_title_on_change = (index)=> {
    ...
    if (wyy_music_cache_log_array.wyy.rank.category[index].cache_time) { // 如果存在缓存
      ...
      new Http({  // 发送http请求
        ...
        // 插入数据到数据库
        await musicStore.db.delete(`DELETE FROM ${rank_title_msg[index].table_name}`) // 新增前先清空表数据
...
const on_update_cache = ()=>{
  ...
  if(Date.now() > time.getNextDayZeroTimestamp(cur_title_msg.cache_time)){  // 可以缓存(如果当前时间戳大于缓存次日起始时间戳)
        new Http({  // 发送http请求
          ...
          // 清空表数据
          musicStore.db.delete(`DELETE FROM ${cur_title_msg.table_name}`).then((result)=>{
            if(result && result.code === 200){
              console.log("表数据已清空！")
