...
<script setup>
  ...
  const apiSqlite = new ApiSqlite(path.resolve(import.meta.env.DEV?"public/":"resources/", "db/music_data.db"));
  ...
  // 在线采集数据本地格式化
  tableData.value = format_line_music_rank_to_local(wyy_array)

  // 本地存储
  // localStorage.setItem("wyy_music_rank_up", JSON.stringify(tableData.value))

  // 排行榜分类切换事件
  const rank_title_on_change = (val)=> {
    cur_rank_title.value = val

    apiSqlite.query("select * from music_rank_wyy_up").then((result)=>{
      console.log(result)
    })

   /* // 发送http请求
    new Http({
      url:'https://music.163.com/discover/toplist?id=19723756', // 域名或 IP 地址。 默认值： 'localhost'。
      method:'get', // 请求方式，默认值： 'get'
    }).go().then(data => {
      const $ = cheerio.load(data);
      tableData.value = JSON.parse($('textarea#song-list-pre-data').text());
    }).catch(err => {
